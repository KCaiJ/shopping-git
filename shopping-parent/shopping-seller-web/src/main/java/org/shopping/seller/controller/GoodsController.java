...
public class GoodsController {
	...
	public Result update(@RequestBody Goods bean, HttpServletRequest request) {
		// 校验是否是当前商家的id
		Goods goods2 = Service.findOne(bean.getGoods().getId());
		// 获取当前登录的商家ID
		String sellerId = CookUtils.getCookieName(request, Enumeration.CURRENT_SELLER);
	...
	public Result isMarketable(Long[] ids, String status) {
		if (Service.isMarketable(ids, status)) {
			return new Result(Enumeration.CODE_SUCCESS, true, Enumeration.UPDATA_SUCCESS);
		}
		return new Result(Enumeration.CODE_SUCCESS, false, Enumeration.UPDATA_FAIL);
	}
